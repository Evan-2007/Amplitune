        ...
        <CardContent className='space-y-6'>
          {servers.map((server) => {
            return (
              <div
                key={server.id}
                className={`flex w-full justify-center rounded-md p-2 ${activeServer && server.id && server.id == activeServer.id ? 'bg-green-600' : 'bg-gray-700'} flex items-center space-x-4`}
              >
                <p>{server.url}</p>

                {activeServer && server.id && server.id == activeServer.id ? (
                  <p>Active</p>
                ) : (
                  <Button onClick={() => setNewServer(server)}>
                    Set Active
                  </Button>
                )}
