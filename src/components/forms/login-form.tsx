  ...
  const onSubmit = async (data: z.infer<typeof schema>) => {
    try {
      ...
      if (response.token) {
        ...
        await localStorage.setItem('activeServer', JSON.stringify({
          url: data.url,
          username: data.username,
          password: stayLoggedIn ? data.password : undefined,
          type: 'navidrome',
    ...
    <Form {...form} >
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 flex justify-center flex-col">
        ...
        <FormField
          control={form.control}
          name="stayLoggedIn"
          render={({ field }) => <FormCheckbox field={field} name="Stay Logged In" />}
        />
        <div>
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <FormLabel className="text-sm font-normal text-center">{name}</FormLabel>
              </TooltipTrigger>
              <TooltipContent>
